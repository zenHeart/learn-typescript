...
unknownVal = null; // 合法
unknownVal = []; // 合法
unknownVal + 1; // 报错, 由于不确定类型无法进行 + 运算
unknownVal.a.v(); // 报错, 不确定类型,所以无法调用此方法
unknownVal.toString(16); // 报错,虽然 unknown 的值为 1,但由于该类型为不确定类型所以操作报错
let bar11: string = unknownVal; // 报错,不确定类型不能复制给确定的类型
let bar12: number = unknownVal; // 报错
let bar13: null = unknownVal; // 报错
let bar14: undefined = unknownVal; // 报错
